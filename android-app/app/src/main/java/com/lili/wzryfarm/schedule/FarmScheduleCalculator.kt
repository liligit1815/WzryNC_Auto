...
object FarmScheduleCalculator {
    ...
    fun calculate(
        ...
        val remainingMinutes = Duration.between(firstWaterAt, observedMaturityAt)
            .toMillis() / 60_000.0
        ...
        val storedRule = cropRules.firstOrNull { it.cycleMinutes == storedCycleMinutes }
            ?.takeIf { remainingMinutes <= it.remainingAfterFirstWater + OCR_MINUTE_TOLERANCE }
        val cycle = (
            storedRule
                ?: cropRules.firstOrNull {
                    remainingMinutes <= it.remainingAfterFirstWater + OCR_MINUTE_TOLERANCE
                }
                ?: cropRules.last()
            ).cycleMinutes
        val batchStart = if (storedRule != null) {
            batchStartedAt ?: firstWaterAt
        } else {
            firstWaterAt
        }
    ...
    private const val OCR_MINUTE_TOLERANCE = 1.0
}
