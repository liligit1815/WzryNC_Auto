...
class FarmActionAutomation(
    ...
    suspend fun run(): FarmActionResult {
        val spawn = runtime.observe(listOf(REFRESH_POSITION))
        ...
        checkNotNull(runtime.observe(listOf(ONE_CLICK_FARM)).matched(ONE_CLICK_FARM)) {
            "一键务农按钮已消失"
        }
        wait(3_000)
        val freshButton = checkNotNull(
            runtime.observe(listOf(ONE_CLICK_FARM)).matched(ONE_CLICK_FARM),
        ) { "等待后未找到一键务农按钮" }
        ...
        val harvested = handleHarvestPopup()
    ...
    private suspend fun handleHarvestPopup(): Boolean {
        repeat(3) { attempt ->
            control.awaitRunnable()
            val popup = runtime.observe(listOf(HARVEST_CONTINUE)).matched(HARVEST_CONTINUE)
            if (popup == null) {
            ...
            val freshPopup = checkNotNull(
                runtime.observe(listOf(HARVEST_CONTINUE)).matched(HARVEST_CONTINUE),
            ) { "收获弹窗在点击前消失" }
            operation("关闭收获弹窗（${freshPopup.centerX}, ${freshPopup.centerY}）")
            runtime.tap(freshPopup.centerX, freshPopup.centerY)
            wait(5_000)
            check(runtime.observe(listOf(HARVEST_CONTINUE)).matched(HARVEST_CONTINUE) == null) {
                "收获弹窗点击后仍然存在"
            }
            return true
        }
    ...
    private suspend fun requireMatch(
        ...
        while (elapsed <= timeoutMs) {
            control.awaitRunnable()
            val screen = runtime.observe(listOf(template))
            ...
            val delay = 3_000L.coerceAtMost(timeoutMs - elapsed)
            wait(delay)
            elapsed += delay
        }
        ...
        repeat(3) { attempt ->
            ...
            when (val reading = runtime.readMaturity()) {
                ...
                is MaturityReading.Unrecognized -> {
                    onLog("土地 OCR 未识别（${attempt + 1}/3）：${reading.reason}")
